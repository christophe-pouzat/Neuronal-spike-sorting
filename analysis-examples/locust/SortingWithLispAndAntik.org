...
We define some utility functions to write data to disk and to plot them using (directly of indirectly) =gnuplot=:
...
#+name: define-plot-raw
#+BEGIN_SRC lisp
  (defun plot-raw (data)
    (let ((nchan (length data)))
      (cgn:format-gnuplot "set border 0")
      (cgn:format-gnuplot "unset xtics")
      (cgn:format-gnuplot "unset ytics")
      (cgn:format-gnuplot "set tmargin 0")
      (cgn:format-gnuplot "set bmargin 0")
      (cgn:format-gnuplot "set lmargin 2")
      (cgn:format-gnuplot "set rmargin 2")
      (cgn:format-gnuplot "unset key")
      (if (> nchan 1)
          (cgn:format-gnuplot (concatenate 'string "set multiplot layout " (format nil "~A" nchan) ",1")))
      (do* ((i 0 (incf i))
            (file-name (concatenate 'string "transient4plot" (format nil "-~A" (1+ i))  ".dat")
                       (concatenate 'string "transient4plot" (format nil "-~A" (1+ i))  ".dat")))
           ((> i (1- nchan)))
        (write-double-2-file (nth i data) file-name)
        (cgn:format-gnuplot (concatenate 'string "plot '" file-name  "' binary format='%float64' using 1 with lines lc rgb 'black'")))
      (if (> nchan 1)
          (cgn:format-gnuplot "unset multiplot"))))
  
  (defun plot-raw-for-org (data generic-file-name fig-name)
    (format t "#+BEGIN_SRC gnuplot :file ~A ~%" fig-name)
    (let ((nchan (length data))
          (file-name generic-file-name))
      (format t "set border 0 ~%")
      (format t "unset xtics ~%")
      (format t "unset ytics ~%")
      (format t "set tmargin 0 ~%")
      (format t "set bmargin 0 ~%")
      (format t "set lmargin 2 ~%")
      (format t "set rmargin 2 ~%")
      (format t "unset key ~%")
      (cond ((= nchan 1)
             (setf file-name (concatenate 'string generic-file-name ".dat"))
             (write-double-2-file (nth 0 data) file-name)
             (format t (concatenate 'string "plot '" file-name "' binary format='%float64' using 1 with lines lc rgb 'black' ~%")))
            (t
             (format t (concatenate 'string "set multiplot layout " (format nil "~A" nchan) ",1 ~%"))
             (dotimes (i nchan)
               (setf file-name (concatenate 'string generic-file-name (format nil "-~A" (1+ i))  ".dat"))
               (write-double-2-file (nth i data) file-name)
               (format t (concatenate 'string "plot '" file-name "' binary format='%float64' using 1 with lines lc rgb 'black' ~%"))
               )
             (format t "unset multiplot ~%"))))
    (format t "#+END_SRC ~%")
    nil)
#+END_SRC

#+BEGIN_SRC lisp :results output raw
  (plot-raw-for-org (mapcar #'window *data-list*) "test-org-plot" "test-fig.png")
#+END_SRC

#+RESULTS:
#+BEGIN_SRC gnuplot :file test-fig.png 
set border 0 
unset xtics 
unset ytics 
set tmargin 0 
...
unset key 
set multiplot layout 4,1 
plot 'test-org-plot-1.dat' binary format='%float64' using 1 with lines lc rgb 'black' 
plot 'test-org-plot-2.dat' binary format='%float64' using 1 with lines lc rgb 'black' 
plot 'test-org-plot-3.dat' binary format='%float64' using 1 with lines lc rgb 'black' 
plot 'test-org-plot-4.dat' binary format='%float64' using 1 with lines lc rgb 'black' 
unset multiplot 
#+END_SRC 
